...
const homePage = async(req, res) => {
    ...
    try {
        ...
        uphone = user[0].phone;
        // console.log(uphone,'is phone');
    } catch (error) {
        console.log(uphone,'in catch')
    }

    return res.render("home/index.ejs", { app ,uphone}); 
}
...
const rechargePage = async(req, res) => {
    let [bank_row] = await connection.execute('SELECT * FROM bank_recharge');
    let bank_detail=bank_row[0]

    return res.render("wallet/recharge.ejs",{ bank_detail }); 
}
