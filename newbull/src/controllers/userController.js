...
const changeUser = async(req, res) => {
    ...
    switch (type) {
        case 'editname':
            ...
            return res.status(200).json({
                message: 'Changed login name successfully',
                // message: 'error in change user 1.5',
                status: true,
                timeStamp: timeNow,
            });
            break;
    ...
    if(data) {
        if(data == 1) {
            ...
            if (check >= data && point_list.total1 != 0) {
                ...
                return res.status(200).json({
                    message: `You have just received ${point_list.total1}.00`,
                    status: true,
                    timeStamp: timeNow,
                });
            } else if (check < get && point_list.total1 != 0) {
                return res.status(200).json({
                    message: 'You are not eligible to receive the gift yet',
                    status: false,
                    timeStamp: timeNow,
                });
        ...
        if(data == 2) {
            ...
            if (check >= get && point_list.total2 != 0) {
                ...
                return res.status(200).json({
                    message: `You have just received ${point_list.total2}.00`,
                    status: true,
                    timeStamp: timeNow,
                });
        ...
        if(data == 3) {
            ...
            if (check >= get && point_list.total3 != 0) {
                ...
                return res.status(200).json({
                    message: `You have just received ${point_list.total3}.00`,
                    status: true,
                    timeStamp: timeNow,
                });
        ...
        if(data == 4) {
            ...
            if (check >= get && point_list.total4 != 0) {
                ...
                return res.status(200).json({
                    message: `You have just received ${point_list.total4}.00`,
                    status: true,
                    timeStamp: timeNow,
                });
        ...
        if(data == 5) {
            ...
            if (check >= get && point_list.total5 != 0) {
                ...
                return res.status(200).json({
                    message: `You have just received ${point_list.total5}.00`,
                    status: true,
                    timeStamp: timeNow,
                });
        ...
        if(data == 6) {
            ...
            if (check >= get && point_list.total6 != 0) {
                ...
                return res.status(200).json({
                    message: `You have just received ${point_list.total6}.00`,
                    status: true,
                    timeStamp: timeNow,
                });
            } else if (check < get && point_list.total6 != 0) {
                return res.status(200).json({
                    message: 'You do not meet the requirements to receive the gift.',
                    status: false,
                    timeStamp: timeNow,
                });
        ...
        if(data == 7) {
            ...
            if (check >= get && point_list.total7 != 0) {
                ...
                return res.status(200).json({
                    message: `You have just received ${point_list.total7}.00`,
                    status: true,
                    timeStamp: timeNow,
                });
...
const recharge = async(req, res) => {
    ...
    let transaction_id_to_save=req.body.transaction_id;

    if (type != 'cancel') {
        if(!auth || !money || money > 10000000) {
            return res.status(200).json({
                // error found in recharge
                message: 'Something went wrong',
                status: false,
                timeStamp: timeNow,
            })
    ...
    if (recharge.length == 0) {
        ...
        if (type == 'momo') {
            ...
            url = ?,
            time = ?`;
            await connection.execute(sql, [client_transaction_id, transaction_id_to_save, userInfo.phone, money, type, 0, checkTime, 'NULL', time]);
            const [recharge] = await connection.query('SELECT * FROM recharge WHERE phone = ? AND status = ? ', [userInfo.phone, 0]);
            return res.status(200).json({
                message: 'Received successfully',
                datas: recharge[0],
        ...
        time = ?`; 
        // set transaction id
        await connection.execute(sql, [client_transaction_id, transaction_id_to_save, userInfo.phone, money, type, 0, checkTime, '0', time]);
        const [recharge] = await connection.query('SELECT * FROM recharge WHERE phone = ? AND status = ? ', [userInfo.phone, 0]);
        return res.status(200).json({
...
const addBank = async(req, res) => {
    ...
    if (user_bank.length == 0 && user_bank2.length == 0) {
        ...
        chi_nhanh = ?,
        time = ?`;
        await connection.execute(sql, [userInfo.phone, '', '', stk, '', '', '', '', '', '']);
        return res.status(200).json({
...
const withdrawal3 = async(req, res) => {
    ...
    let money = req.body.money;
    let password = req.body.password;
    if(!auth || !money || !password || money > 2000000) {
        return res.status(200).json({
            message: 'Failed',
            status: false,
            timeStamp: timeNow,
    ...
    console.log(total,'is total next is 2 total ' ,total2)
    let result = 0;
    if(total - total2 > 0) result = total - total2;
    
    console.log('old money is ',userInfo.money)
    console.log('user phone is ',userInfo.phone)
    const [current_user]= await connection.execute(`SELECT * FROM users WHERE phone = ?`,[userInfo.phone]);
    console.log(current_user[0].playmoney,'is play money')
    if (money > (userInfo.money-current_user[0].playmoney) ) {
        return res.status(200).json({           
            message: `You Can Only Withdraw upto ${userInfo.money-current_user[0].playmoney}`,
            status: false,
            timeStamp: timeNow,
        });
    }
        ...
        if (withdraw.length < 3) {
            ...
            if ((userInfo.money - money) >= 0) {
                if (result >= 0) {
                    ...
                    today = ?,
                    time = ?`;
                    await connection.execute(sql, [id_time + '' + id_order, userInfo.phone, money, infoBank.stk, '', '', 0, checkTime, dates]);
                ...
                return res.status(200).json({
                    // withdraw error found   // The balance is insufficient to fulfill the request
            ...
            return res.status(200).json({
                message: 'You are only allowed to withdraw money 3 times per day.',
                status: false,
                timeStamp: timeNow,
            });
        }
...
const useRedenvelope = async(req, res) => {
    ...
    if (redenvelopes.length == 0) {
        ...
        if (infoRe.status == 0) {
            ...
            return res.status(200).json({
                message: 'Gift code has already been used',
                status: false,
                timeStamp: timeNow,
            });
        }
