...
const rechargeDuyet = async(req, res) => {
    ...
    if (type == 'confirm') {
        ...
        let [thisuser]=await connection.query(`SELECT * FROM users WHERE phone = ? `,[info[0].phone])
        let reffercode=thisuser[0].invite
    
    
       

        const [history]=await connection.query(`SELECT * FROM recharge WHERE phone = ?`, [info[0].phone]);
       
        let tomoney=(info[0].money) +( (info[0].money*5)/100);
        let prc=(info[0].money*5)/100
   
        if (history.length >1) {
            await connection.query('UPDATE users SET money = money + ?, playmoney = playmoney + ?,total_money = total_money + ? WHERE phone = ? ', [info[0].money, info[0].money,info[0].money, info[0].phone]);
        }
        else{

            await connection.query('UPDATE users SET money = money + ?, playmoney = playmoney + ?,total_money = total_money + ? WHERE phone = ? ', [tomoney,tomoney, tomoney, info[0].phone]);
          
        }
        await connection.query('UPDATE users SET money = money + ?, total_money = total_money + ? WHERE code = ? ', [prc, prc, reffercode]);
        return res.status(200).json({
