...
const rosesPlus = async (auth, money) => {
    ...
    const [user] = await connection.query('SELECT `phone`, `code`, `invite`, `level`, `total_money` FROM users WHERE token = ? AND veri = 1 LIMIT 1 ', [auth]);

    let userInfo = user[0];
    
    const [f1] = await connection.query('SELECT `phone`, `code`, `invite`, `rank`, `level`, `total_money` FROM users WHERE code = ? AND veri = 1 LIMIT 1 ', [userInfo.invite]);
    ...
    if (userInfo.total_money >= 100) {
        if (f1.length > 0) {
            ...
            for (let levelIndex = 1; levelIndex <= 6; levelIndex++) {
                let rosesF = 0;
                if (infoF1.level >= levelIndex && infoF1.total_money >= 100) {
                ...
                const [fNext] = await connection.query('SELECT `phone`, `code`, `invite`, `rank`, `level`, `total_money` FROM users WHERE code = ? AND veri = 1 LIMIT 1 ', [infoF1.invite]);
                if (fNext.length > 0) {
                    infoF1 = fNext[0];
                } else {
                    break;
...
const betWinGo = async (req, res) => {
    ...
    if (check >= 0) {
        ...
        let playmoney=current_user[0].playmoney
        console.log(playmoney,'is play money')
        if ( (money*x) <= playmoney ) {
            await connection.execute('UPDATE `users` SET `playmoney` = `playmoney` - ? WHERE `token` = ? ', [money * x, auth]);
          
        }else{
            await connection.execute('UPDATE `users` SET `playmoney` = ? WHERE `token` = ? ', [0, auth]);  
        }
