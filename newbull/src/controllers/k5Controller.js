...
const betK5D = async (req, res) => {
    try {
        ...
        if (check >= 0) {
            ...
            const [current_user]=await connection.execute(`SELECT * FROM users WHERE token = ?`,[auth]);
            let playmoney=current_user[0].playmoney
            console.log(playmoney,'is play money')
            if ( total <= playmoney ) {
                await connection.execute('UPDATE `users` SET `playmoney` = `playmoney` - ? WHERE `token` = ? ', [total, auth]);
